...
void exit_program() {
  cout << "Thanks for using Quickie Mobile Money Service. Goodbye!" << endl;
  exit(0);
}
...
void send_money(int& balance) {
  ...
  if (amount <= balance) {
    balance -= amount;
    cout << "You have sent " << amount << " to the receiver. Your current balance is " << balance << endl;
  }
  else {
...
int main() {
  ...
  while (true) {
    cout << "Welcome to QUICKIE Mobile Money Service! " << endl;
    cout << "A Reliable, safe and quick way to send money to friends and families outside and inside your domicile! Please select an option:" << endl;
